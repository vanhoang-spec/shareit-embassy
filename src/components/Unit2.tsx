...
export function AlienTriviaArena({ onBack, addCoins, unit = 1, level = "level_5" }) {
  ...
  if (questions.length === 0) {
    return (
      <div className="ac-fade"><BackBar onBack={onBack} color="#f472b6" />
        <div className="rounded-3xl gx-glass p-6 text-center text-indigo-100"><p className="text-lg font-black text-white">👾 Alien Trivia</p><p className="mt-2 text-sm">This unit has no quiz content yet.</p></div>
      </div>
    );
  }
  const cur = questions[idx];
  ...
  return (
    <div className="ac-fade"><BackBar onBack={onBack} color="#f472b6" />
      ...
      {!done ? (
        <>
          <div className="mb-4 rounded-3xl gx-glass p-5 text-center"><p className="text-[11px] font-bold text-indigo-300">Question {idx + 1} / {questions.length}</p><p className="mt-2 text-lg font-black leading-snug text-white">{cur.emoji} {cur.q}</p></div>
          <div className="space-y-2">
            {cur.opts.map((opt, i) => {
              const isPick = picked === opt, isRight = picked !== null && opt === cur.a, isWrong = isPick && opt !== cur.a;
              return (<button key={i} onClick={() => choose(opt)} className="w-full rounded-2xl px-4 py-3 text-left text-sm font-black text-white transition active:scale-95" style={{ background: isRight ? "linear-gradient(135deg,#10b981,#34d399)" : isWrong ? "linear-gradient(135deg,#f43f5e,#fb7185)" : "rgba(255,255,255,.08)", boxShadow: isPick ? "0 0 14px #f472b688" : "inset 0 0 0 1px rgba(255,255,255,.18)" }}>{opt}</button>);
            })}
          </div>
        </>
      ) : (
        <div className="rounded-3xl gx-glass p-6 text-center"><p className="text-2xl font-black text-white">🎉 Quiz complete!</p><p className="mt-1 text-base font-black text-pink-200">Score: {score} / {questions.length}</p><p className="mt-1 text-sm font-bold text-emerald-200">+15 cosmic coins</p><button onClick={reset} className="mt-4 rounded-full bg-white/10 px-5 py-2 text-sm font-black text-white ring-1 ring-white/20 hover:bg-white/20">Play again 🔄</button></div>
      )}
    </div>
  );
}
...
export function FutureWillWont({ onBack, addCoins, unit = 1, level = "level_5" }) {
  ...
  const awarded = useRef(false);
  if (rounds.length === 0) {
    return (<div className="ac-fade"><BackBar onBack={onBack} color="#34d399" /><div className="rounded-3xl gx-glass p-6 text-center text-indigo-100"><p className="text-lg font-black text-white">🔮 Will or Won't?</p><p className="mt-2 text-sm">This unit has no will/won't sentences yet.</p></div></div>);
  }
  ...
  function reset() { setIdx(0); setPick(null); setScore(0); setDone(false); awarded.current = false; }
  return (
    <div className="ac-fade"><BackBar onBack={onBack} color="#34d399" />
      ...
      {!done ? (
        <>
          <div className="mb-4 rounded-3xl gx-glass p-5 text-center"><p className="text-[11px] font-bold text-indigo-300">Sentence {idx + 1} / {rounds.length}</p><p className="mt-2 text-lg font-black leading-snug text-white">{cur.before} <span className="mx-1 rounded-md bg-white/15 px-3 py-0.5 text-emerald-200">____</span> {cur.after}</p></div>
          <div className="grid grid-cols-2 gap-3">
            {["will", "won't"].map((opt) => {
              const isPick = pick === opt, isRight = pick !== null && opt === cur.answer, isWrong = isPick && opt !== cur.answer;
              return (<button key={opt} onClick={() => choose(opt)} className="rounded-2xl px-4 py-5 text-center text-lg font-black text-white transition active:scale-95" style={{ background: isRight ? "linear-gradient(135deg,#10b981,#34d399)" : isWrong ? "linear-gradient(135deg,#f43f5e,#fb7185)" : "rgba(255,255,255,.08)", boxShadow: isPick ? "0 0 14px #34d39988" : "inset 0 0 0 1px rgba(255,255,255,.18)" }}>{opt}</button>);
            })}
          </div>
        </>
      ) : (
        <div className="rounded-3xl gx-glass p-6 text-center"><p className="text-2xl font-black text-white">🎉 Great future talk!</p><p className="mt-1 text-base font-black text-emerald-200">Score: {score} / {rounds.length}</p><p className="mt-1 text-sm font-bold text-emerald-200">+15 cosmic coins</p><button onClick={reset} className="mt-4 rounded-full bg-white/10 px-5 py-2 text-sm font-black text-white ring-1 ring-white/20 hover:bg-white/20">Play again 🔄</button></div>
      )}
    </div>
  );
}
