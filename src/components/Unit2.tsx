      ...
      {!done ? (
        <>
          ...
          <div className="space-y-2">
            {cur.opts.map((opt, i) => {
              const isPick = picked === opt,
                isRight = picked !== null && opt === cur.a,
                isWrong = isPick && opt !== cur.a;
              return (
                <button
                  key={i}
                  onClick={() => choose(opt)}
...
// ---------- PLANET 2: Data-driven grammar — Will or Won't? ----------
...
// ---------- PLANET 2 (Case B): Rocket Sentence Blaster — data-driven arcade for ALL units ----------
export function RocketSentenceBlaster({ onBack, addCoins, unit = 1, level = "level_5" }) {
  const ud = getUnitData(unit, level);

  // Rounds. Priority 1: sentence-completion from grammar.willWont.
  //         Priority 2: word-transform from vocabulary.lesson_1 (word -> alt), like Balloon Pop.
  const { mode, rounds } = useMemo(() => {
    const ww = ud.grammar?.willWont;
    if (Array.isArray(ww) && ww.length > 0) {
      const answers = [...new Set(ww.map((s) => s.answer).filter(Boolean))];
      const rs = ww.map((s) => {
        const opts = shuffle(answers.filter((a) => a !== s.answer)).slice(0, 3);
        const options = shuffle([s.answer, ...opts]);
        return { before: s.before, after: s.after, answer: s.answer, options };
      });
      return { mode: "sentence", rounds: rs };
    }
    const items = (ud.vocabulary?.lesson_1 || []).filter((c) => c && c.alt && c.alt !== c.word);
    const rs = items.map((it, ri) => {
      const otherAlts = shuffle(items.filter((_, k) => k !== ri).map((o) => o.alt));
      // base-form trap: the unchanged word is the key distractor (child must APPLY the change)
      const distract = [...new Set([it.word, ...otherAlts.filter((o) => o !== it.alt && o !== it.word)])].slice(0, 3);
      const options = shuffle([it.alt, ...distract]);
      return { prompt: it.word, emoji: it.emoji, answer: it.alt, options };
    });
    return { mode: "word", rounds: rs };
  }, [unit, level]); // eslint-disable-line react-hooks/exhaustive-deps

  const COIN = 5; // coins per correct hit / penalty per wrong hit
  const [idx, setIdx] = useState(0);
  const [coins, setCoins] = useState(0); // session coins (awarded to profile on win)
  const [flash, setFlash] = useState(null); // {ok, key}
  const [shot, setShot] = useState(null); // {angle, len} laser aimed at target
  const [aim, setAim] = useState(0); // rocket nose angle (deg)
  const [boom, setBoom] = useState(null); // option index exploding
  const [rocketBoom, setRocketBoom] = useState(false); // rocket explodes on wrong hit
  const [pop, setPop] = useState(null); // {ok, x, y} floating coin/penalty popup
  const [done, setDone] = useState(null); // 'win'
  const awarded = useRef(false);
  const playRef = useRef(null);
  const acRef = useRef(null);

  // Synthesized sound effects (Web Audio) — no asset files needed.
  function playSfx(type) {
    try {
      ...
      const tone = (f0, f1, dur, gain, wave, startAt) => {
        const t0 = now + (startAt || 0);
      ...
      if (type === "laser") tone(880, 120, 0.18, 0.2, "sawtooth");
      else if (type === "explode") noise(0.4, 0.5);
      else if (type === "coin") {
        tone(988, 988, 0.08, 0.22, "square");
        tone(1319, 1319, 0.12, 0.22, "square", 0.08);
      } else if (type === "fail") {
        tone(400, 70, 0.5, 0.28, "sawtooth");
        noise(0.45, 0.35, 0.02);
      } else if (type === "win") {
        [523, 659, 784, 1047].forEach((f, k) => tone(f, f, 0.16, 0.2, "triangle", k * 0.13));
      }
    } catch {}
  }
  ...
  if (rounds.length === 0) {
    return (
      <div className="ac-fade">
        <BackBar onBack={onBack} color="#fb7185" />
        <div className="rounded-3xl gx-glass p-6 text-center text-indigo-100">
          <p className="text-lg font-black text-white">🚀 Rocket Sentence Blaster</p>
          <p className="mt-2 text-sm">This unit has no grammar or word-change data yet.</p>
        </div>
      </div>
    );
  }
  ...
  const layout = useMemo(() => {
    ...
    return cur.options.map((opt, i) => ({
      opt,
      left: ((i + 1) * 100) / (n + 1), // even spread across the FULL width
      dur: 5.5 + ((i * 1.3) % 3),
      delay: -(i * 1.4),
    }));
  }, [idx]); // eslint-disable-line react-hooks/exhaustive-deps

  function fire(i, opt, e) {
    if (flash || done) return;
    const ok = opt === cur.answer;
    // Aim the laser + rocket nose at the asteroid's live (animated) position.
    let tx = null;
    let ty = null;
    try {
      ...
      if (pr && tr) {
        const Rx = pr.width / 2;
        const Ry = pr.height - 24; // rocket pivot near bottom-center
        tx = tr.left + tr.width / 2 - pr.left;
        ty = tr.top + tr.height / 2 - pr.top;
        const dx = tx - Rx;
        const dy = ty - Ry;
        const angle = (Math.atan2(dx, -dy) * 180) / Math.PI; // 0 = straight up, + = right
        setShot({ angle, len: Math.max(40, Math.hypot(dx, dy)) });
        setAim(angle);
      } else {
        setShot({ angle: 0, len: 260 });
      }
    } catch {
      setShot({ angle: 0, len: 260 });
    }
    ...
    if (ok) {
      ...
      setCoins(newCoins);
      setBoom(i); // asteroid explodes
      setPop({ ok: true, x: tx, y: ty });
      setTimeout(() => {
        playSfx("explode");
        playSfx("coin");
      }, 170);
      try {
        speak(mode === "sentence" ? `${cur.before} ${cur.answer} ${cur.after}` : cur.answer);
      } catch {}
      setTimeout(() => {
        setShot(null);
        setAim(0);
        ...
        if (idx + 1 >= rounds.length) {
          if (!awarded.current) {
            ...
            playSfx("win");
            try {
              confetti({ particleCount: 200, spread: 110, startVelocity: 55, origin: { y: 0.6 } });
            } catch {}
          }
          setDone("win");
        } else {
      ...
      const newCoins = Math.max(0, coins - COIN);
      setCoins(newCoins);
      setRocketBoom(true); // the ROCKET explodes
      setPop({ ok: false });
      setTimeout(() => playSfx("fail"), 170);
      setTimeout(() => {
        setShot(null);
        setAim(0);
        setFlash(null);
        setPop(null);
        setRocketBoom(false);
        setIdx(0); // wrong answer -> restart from the first question
      }, 850);
    }
  }
  ...
  return (
    <div className="ac-fade">
      <BackBar onBack={onBack} color="#fb7185" />
      <style>{`
        @keyframes rsbDrift { from { transform: translate(-50%, -18%); } to { transform: translate(-50%, 460%); } }
        @keyframes rsbBeam { 0% { transform: scaleY(0); opacity:.3 } 55% { transform: scaleY(1); opacity:1 } 100% { transform: scaleY(1); opacity:0 } }
        @keyframes rsbBoom { 0% { transform: scale(.3); opacity:1 } 100% { transform: scale(1.9); opacity:0 } }
        @keyframes rsbFloat { 0% { transform: translate(-50%,-50%) scale(.6); opacity:0 } 30% { opacity:1 } 100% { transform: translate(-50%,-150%) scale(1.1); opacity:0 } }
        @keyframes rsbStars { from { background-position: 0 0 } to { background-position: 0 240px } }
      `}</style>

      <div className="mb-3 rounded-3xl p-4 text-center gx-glass" style={{ boxShadow: "0 0 18px #fb718566" }}>
        <p className="text-[11px] font-extrabold uppercase tracking-widest text-rose-300">
          Planet 2 · Grammar Black Hole
        </p>
        <p className="text-xl font-black text-white">🚀 Rocket Sentence Blaster</p>
        <p className="text-[11px] font-bold text-indigo-200">
          {mode === "sentence"
            ? "Blast the word that completes the sentence!"
            : `Blast the correct ${ud.formToggleLabel || "form"}!`}
        </p>
      </div>

      {!done ? (
        <>
          <div className="mb-2 flex items-center justify-between text-xs font-bold text-indigo-200">
            <span>
              Target {idx + 1} / {rounds.length}
            </span>
            <span className="text-amber-300">🪙 {coins}</span>
          </div>

          <div className="mb-2 rounded-2xl gx-glass p-3 text-center">
            {mode === "sentence" ? (
              <p className="text-lg font-black text-white">
                {cur.before}{" "}
                <span className="mx-1 rounded-md bg-rose-500/30 px-3 py-0.5 text-rose-200 ring-1 ring-rose-300/40">
                  ?
                </span>{" "}
                {cur.after}
              </p>
              ...
              <p className="text-lg font-black text-white">
                Change: <span className="text-2xl">{cur.emoji}</span> {cur.prompt}
              </p>
            )}
          </div>
          ...
          <div
            ...
            style={{
              height: 360,
              background: "radial-gradient(120% 100% at 50% 0%, #1e1b4b 0%, #0b1024 70%, #05030f 100%)",
              boxShadow: "inset 0 0 40px #000a, 0 0 18px #312e8166",
            }}
          >
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(1px 1px at 20px 30px, #fff, transparent), radial-gradient(1px 1px at 120px 80px, #cbd5e1, transparent), radial-gradient(1.5px 1.5px at 200px 160px, #fff, transparent), radial-gradient(1px 1px at 80px 220px, #93c5fd, transparent)",
                backgroundSize: "220px 240px",
                animation: "rsbStars 6s linear infinite",
              }}
            />

            {layout.map((a, i) => {
              const isBoom = boom === i;
              const isWrong = flash && flash.key === i && !flash.ok;
              const dimmed = flash && flash.key !== i;
              return (
                <button
                  key={`${idx}-${i}`}
                  onClick={(e) => fire(i, a.opt, e)}
                  className="absolute top-0 flex items-center justify-center rounded-full px-3 text-center text-sm font-black text-white active:scale-90"
                  style={{
                    left: `${a.left}%`,
                    minWidth: 64,
                    height: 64,
                    animation: `rsbDrift ${a.dur}s linear ${a.delay}s infinite`,
                    background: isWrong
                      ? "radial-gradient(circle at 35% 30%, #fda4af, #be123c)"
                      : "radial-gradient(circle at 35% 30%, #fde68a, #b45309 70%)",
                    boxShadow: isWrong ? "0 0 18px #f43f5e" : "0 0 14px #f59e0baa, inset -4px -6px 10px #0006",
                    filter: dimmed ? "grayscale(.5) opacity(.55)" : "none",
                  }}
                >
                  ☄️ {a.opt}
                  {isBoom && (
                    <span
                      ...
                      style={{ animation: "rsbBoom .5s ease-out forwards" }}
                    >
                      💥
                    </span>
                  )}
              ...
              >
                <div
                  style={{
                    ...
                    height: shot.len,
                    transformOrigin: "bottom center",
                    background: "linear-gradient(to top, #f0abfc, #22d3ee)",
                    boxShadow: "0 0 12px #f0abfc",
                    animation: "rsbBeam .4s ease-out forwards",
                  }}
                />
              </div>
            )}

            {pop && pop.ok && pop.x != null && (
              <div
                className="pointer-events-none absolute text-base font-black text-amber-300"
                style={{
                  left: pop.x,
                  top: pop.y,
                  transform: "translate(-50%,-50%)",
                  animation: "rsbFloat .7s ease-out forwards",
                }}
              >
                +{COIN} 🪙
              </div>
            ...
            {pop && !pop.ok && (
              <div
                className="pointer-events-none absolute bottom-14 left-1/2 text-base font-black text-rose-300"
                style={{ transform: "translateX(-50%)", animation: "rsbFloat .8s ease-out forwards" }}
              >
                -{COIN} 🪙
              </div>
            )}
            ...
            <div className="absolute bottom-2 left-1/2" style={{ transform: "translateX(-50%)" }}>
              {rocketBoom ? (
                <span className="inline-block text-5xl" style={{ animation: "rsbBoom .8s ease-out forwards" }}>
                  💥
                </span>
              ) : (
                <span
          ...
          <p className="mt-2 text-center text-[11px] font-bold text-indigo-300">
            Tap the asteroid carrying the correct answer to blast it! ☄️💥
          </p>
        </>
      ) : (
        <div className="rounded-3xl gx-glass p-6 text-center" style={{ boxShadow: "0 0 24px #fb7185" }}>
          <div className="text-5xl">🚀💥🌟</div>
          <p className="mt-2 text-2xl font-black text-white">All targets blasted!</p>
          <p className="mt-1 text-sm font-bold text-amber-200">+{coins} 🪙 cosmic coins</p>
          <button
            onClick={restart}
            className="mt-4 rounded-full bg-white/10 px-5 py-2 text-sm font-black text-white ring-1 ring-white/20 hover:bg-white/20"
          >
